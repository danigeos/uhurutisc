	PROGRAM UHURU
!!   Data inici: 1996.    Ultima modifica: 10/02/2014
!! Include the calculation of the Total Strain accumulated. It is saved in a file.
...
!!		   res_StrainRateEff*   open(30) Effective strain rate
!!		   res_TotalStrain*	Total Stran accumulation
...
!!	hydro_model=1 for rain proportional to elevation; hydro_model=2/3 for orographic precipitation (wind-dependent)
...
!!		type *, system ('mv '//BCfile//' pippo')
     ...
     +		   file_TK,file_ep, file_TStr
      CHARACTER*1 file_ext1
      CHARACTER*2 file_ext2
      CHARACTER*3 file_ext3
      LOGICAL RESTART
     ...
     +          visinf=1.D21, Depth_lit=120.D3, irel_ZL=2, 
     +		ZLITOS_rel=130.D3, DGL_ter=20.D3, Ldepth_var=1, 
     +		iTemp_var=1, ifixv=1)
      DATA iw_erosion /0/, vissup /1.D25/
      REAL dt_eros, Kerosdif, K_river_cap, erodability, 
     +	     erodability_sed, l_fluv_sedim,rain, Krain, windazimut, 
     +	     relative_humidity, CXrain, CYrain, evaporation, 
     +	     xmin, xmax, ymin, ymax, Dt_tisc, Te_flexure
      INTEGER*2 Nx, Ny, hydro_model
      PARAMETER (dt_eros=0.01)
     ...
     +		fin_rotat, A_removal, B_removal, Delta_sed_eros,
     +          TotalStrain
     ...
     +            visco_cnst, Iremoval, Time_removal, Zremoval,
     +            TISO_rem, Zctall, iremesh, dvis_allow, Dif_K,  
     +		  Te_flexure, hydro_model, Kerosdif, rain, Krain, 
     +		  windazimut, relative_humidity, evaporation, 
     +		  erodability, erodability_sed, K_river_cap, 
     +		  l_fluv_sedim, CXrain, CYrain)
     
   !   dGLit_allow=dvis_allow
	dGLit_allow=1.D10	!!!!  without filtering lithosphere thickness and Temperature distribution.
      Dif_K_L=Dif_K
...
!!   INPUT_DATA=3 ==>  x, y, crustal and lithospheric thickness
     ...
     +            I4,'  m =',I4)") n_file,m_file
	  ...
	  ALLOCATE(Delta_sed_eros(nn),STAT=istat2)
	  ...
	  ALLOCATE(TotalStrain(nn),STAT=istat8)
...
C --------------------------------------------------------------------
	...
	fin_rotat=0.D0
     ...
     +		l_fluv_sedim, hydro_model, rain, Krain, windazimut, 
     +		relative_humidity, CXrain, CYrain, 
     +		evaporation, Te_flexure)
      ENDIF
...
1000     CONTINUE 
      ...
      IF(npasos==NPASKEEP.OR.npasos==npasmax.OR.npasos==1) THEN
	   ...
	   nres_cent1=AINT((nresults-48.D0)/100.D0)
	   nres_unit=nresults-10*nres_dec1
	   nres_dec=nres_dec1+48-10*nres_cent1
	   nres_cent=nres_cent1+48
	   file_ext1=CHAR(nres_unit)
	   file_ext2=CHAR(nres_dec)//CHAR(nres_unit) 
	   file_ext3=CHAR(nres_cent)//CHAR(nres_dec)//CHAR(nres_unit) 
      ENDIF
     ...
     +			 epuntzz, TotalStrain, visco, Tmoho, Qsurface,
...
!!      Sediment thickness
	   ...
	   IF(sedmax>1.D-100) THEN
	       IBC_thicken=1   !  0 => No lateral variations of the thickening on the boundaries, d(thickness)/dx=d(thickness)/dy=0
...
!!      Total erosion
	   ...
	   IF(erosmax>1.D-100) THEN
	       IBC_thicken=1   !  1 => d(thickness)/dx=d(thickness)/dy=0  (boundaries)
	       IF(IBC_thicken==0) THEN
...
!!	Total Strain Accumulation
           CALL THICKEN (Dtsegons, n, m, nn, AX, BY, u, v, epuntzz,
     +				TotalStrain, IBC_thicken)
	   AverageStrain=0.D0
           DO kxy=1,nn
		!! TotalStrain(kxy)=TotalStrain(kxy)+Dtsegons*epuntzz(kxy)
		AverageStrain=AverageStrain+TotalStrain(kxy)
	   END DO
	   AverageStrain=AverageStrain/nn
 ...
 !    +			 Tmoho, TISOTER, Dx, Dy, kpuntssd, D_Bodies)
     ...
     +			TISO_rem, Zctall, Niremoved,
     +			A_removal, B_removal, Delta_sed_eros)
	   ENDIF
	   DEALLOCATE(TEMPE1)  
     ...
     +		     ' GLit_ter minim',F8.3,' km')") GLmax/1D3,GLmin/1D3
           ...
           IF(iwrite==1) THEN
             IF(nres_cent==48) THEN
                IF(nres_dec==48) THEN
                  file_Pav='res_Szzaver_Flit'//file_ext1
		  file_ep='res_StrainRateEff'//file_ext1
                ELSE
                  file_Pav='res_Szzaver_Flit'//file_ext2
		  file_ep='res_StrainRateEff'//file_ext2
                ENDIF
             ELSE
                file_Pav='res_Szzaver_Flit'//file_ext3
                file_ep='res_StrainRateEff'//file_ext3
             ENDIF
           ENDIF
          ALLOCATE(Flitos(nn),STAT=istat3) 
		Flitos=0.D0
...
!		END IF
     ...
     +			  Tmoho, TISOTER, ZASTH, Delta_sed_eros)
	...
	TEMPE1=TEMPE2	! re-initialize the temperature for the next time step
        DEALLOCATE(TEMPE2)
          IF(iwrite==1.AND.npasos/=npasmax) THEN
            IF(nres_cent==48) THEN
               IF(nres_dec==48) THEN
                  file_result='resultats'//file_ext1
                  file_eros='res_erosion'//file_ext1
                  file_rot='res_rotation'//file_ext1
                  file_TK='res_Tempe'//file_ext1
		  file_TStr='res_TotalStrain'//file_ext1
               ELSE
                  file_result='resultats'//file_ext2
                  file_eros='res_erosion'//file_ext2
                  file_rot='res_rotation'//file_ext2
                  file_TK='res_Tempe'//file_ext2
		  file_TStr='res_TotalStrain'//file_ext2
               ENDIF
            ELSE
               file_result='resultats'//file_ext3
               file_eros='res_erosion'//file_ext3
               file_rot='res_rotation'//file_ext3
               file_TK='res_Tempe'//file_ext3
               file_TStr='res_TotalStrain'//file_ext3
            ENDIF
     ...
     +				(iy*Dy,iz*Dz,TEMPE1(kxy,iz), iz=0,NELZ)
		 ...
		 ENDIF		    
             ...
             OPEN(30,FILE=file_TStr)					!! Total Strain Accumulation
             WRITE(30,"('# Time:',F6.2,' My.   x[m], y[m],',
     +		' Total Strain Accumulation. Average Total Strain =', 
     +			F17.6)") TEMPSMa, AverageStrain
      	        DO iy=0,m
		   DO ix=0,n
		      kxy=ix+1+iy*(n+1)
                      WRITE(30,"(2F15.3,G19.8)") 
     +				ix*Dx,iy*Dy,TotalStrain(kxy)
		   END DO
	        END DO
	     CLOSE (30)
...
!!  Niremoved : Number of geotherms that have been convective removed
     ...
     +		A_removal(nn), B_removal(nn), Delta_sed_eros(nn)
      CHARACTER*55 zname
...
!	      TADVECminP=VAL_MIN(TADVEC,nn)
	     ...
	     HSURF_fe=HSURF
	     !HEXP_fe=HEXP
	     HEXP_fe=(HEXP/35.D3)*ZMOHO		!!! Exponential decay scaled with ZMOHO
...
!     +				Temp_z,TISOTER,DGL_ter,NELZ,Dz,Iremoved)
		    ...
		    IF(Iremoved==1) WRITE(6,"(3X,'ConvectiveRemoval: ',
     + 			 2F10.2, ' km')") (ix*Dx)/1D3, (iy*Dy)/1D3
		END IF	
...
!!     All variables under S.I.
	...
	nbodies=0
     ...
     +             kpunts3,qFlit3
	  ...
	  READ(12,*,END=440) kpunts4,qFlit4	!! BODY 4
	  WRITE (6,"('Body 4: ',I4,' nodes.  qFlit4:',F11.2)")
     +             kpunts4,qFlit4
	  D_Bodies(kpunts1+kpunts2+kpunts3+4,1)=kpunts4
	  D_Bodies(kpunts1+kpunts2+kpunts3+4,2)=qFlit4
	  nbodies=nbodies+1
	  DO I=kpunts1+kpunts2+kpunts3+5,
     +		kpunts1+kpunts2+kpunts3+kpunts4+4
	  ...
	  READ(12,*,END=440) kpunts5,qFlit5	!! BODY 5
	  WRITE (6,"('Body 5: ',I4,' nodes.  qFlit5:',F11.2)")
     +             kpunts5,qFlit5
	  D_Bodies(kpunts1+kpunts2+kpunts3+kpunts4+5,1)=kpunts5
	  D_Bodies(kpunts1+kpunts2+kpunts3+kpunts4+5,2)=qFlit5
	  nbodies=nbodies+1
	  DO I=kpunts1+kpunts2+kpunts3+kpunts4+6,
 ...
 440	  WRITE(6,"(3X,'Number of bodies:',I3)") nbodies
     ...
     +					m, n, nn, Dx, Dy, u, v, Dtsegons)
                ...
                D_Bodies(kp,2)=position_y
          END DO
	  kpunts4=D_Bodies(kpunts1+kpunts2+kpunts3+4,1)
          DO kp=kpunts1+kpunts2+kpunts3+5,
     +			kpunts1+kpunts2+kpunts3+kpunts4+4	!! BODY 4
                position_x=D_Bodies(kp,1)
		position_y=D_Bodies(kp,2)
		CALL new_position (position_x, position_y,
     +					m, n, nn, Dx, Dy, u, v, Dtsegons)
                ...
                D_Bodies(kp,2)=position_y
          END DO
	  kpunts5=D_Bodies(kpunts1+kpunts2+kpunts3+kpunts4+5,1)
          DO kp=kpunts1+kpunts2+kpunts3+kpunts4+6,
     +			kpunts1+kpunts2+kpunts3+kpunts4+kpunts5+5	!! BODY 5
                position_x=D_Bodies(kp,1)
		position_y=D_Bodies(kp,2)
		CALL new_position (position_x, position_y,
     +					m, n, nn, Dx, Dy, u, v, Dtsegons)
...
! iswitch=1 => keep in a file the stress envelope and the strength
      ...
      DOUBLE PRECISION, DIMENSION(:), ALLOCATABLE :: x_pol1, y_pol1,
     +		x_pol2,y_pol2, x_pol3,y_pol3, x_pol4,y_pol4,
     +		x_pol5,y_pol5, T, ep_eff, ep_visc, ibody
...
!     +		Tmoho_tmp(nn), Qsurface_tmp(nn),vis_input(nn)
      ...
      WRITE(6,"('CALCULATION OF THE VISCOSITY')") 
       ...
       ALLOCATE(x_pol4(kpuntssd),STAT=istat5) 
       ALLOCATE(y_pol4(kpuntssd),STAT=istat6) 
       ALLOCATE(x_pol5(kpuntssd),STAT=istat5) 
       ALLOCATE(y_pol5(kpuntssd),STAT=istat6) 
...
!      Qsurface_tmp=70.D-3
	...
	DO kp=kpunts1+kpunts2+kpunts3+5,
     +		kpunts1+kpunts2+kpunts3+kpunts4+4	
	    x_pol4(i)=D_Bodies(kp,1)
	    y_pol4(i)=D_Bodies(kp,2)
	    i=i+1
	END DO
	...
	DO kp=kpunts1+kpunts2+kpunts3+kpunts4+6,
     +		kpunts1+kpunts2+kpunts3+kpunts4+kpunts5+5	
	    x_pol5(i)=D_Bodies(kp,1)
	    y_pol5(i)=D_Bodies(kp,2)
	    i=i+1
	END DO
     ...
     +	     ', STRENGTH MULTIPLIED BY qFlit3=',F11.2)")kpunts3,qFlit3
	   IF(kpunts4/=0) WRITE(6,"(3X,'BODY 4: kpunts4=',I3,
     +	     ', STRENGTH MULTIPLIED BY qFlit4=',F11.2)")kpunts4,qFlit4
	   IF(kpunts5/=0) WRITE(6,"(3X,'BODY 5: kpunts5=',I3,
     +	     ', STRENGTH MULTIPLIED BY qFlit5=',F11.2)")kpunts5,qFlit5
      ENDIF
CCC -------------------------------------------------------------------
     ...
     +					ix, iy, PX/1.D3, PY/1.D3	!! Points where the velocity is fixed
		...
		ENDIF
		IF(kpunts4/=0) THEN
           	    CALL outin (PX,PY,kpunts4,x_pol4,y_pol4,iadentro)
               	    IF(iadentro==1) visco(kxy)=visco_cnst*qFlit4
		ENDIF
		IF(kpunts5/=0) THEN
           	    CALL outin (PX,PY,kpunts5,x_pol5,y_pol5,iadentro)
               	    IF(iadentro==1) visco(kxy)=visco_cnst*qFlit5
		ENDIF
	 ...
	 IF(ITSR/=1.AND.TEMPSMa/=0.D0) THEN
	     DO iy=0,m
		DO ix=0,n
		   kxy=ix+1+iy*(n+1)		!! The function is limited in the library RW_PARAMETRES.f (e.g: epeffec_inf=1.D-20, epeffec_sup=1.D-13)
...
!! Lithosphere Strength --> Viscosity
	...
	!!     IF(ibody(kxy)==1) visco(kxy)=1.D25		!! TIBETAN PLATEAU
      ...
      DO kxy=1,nn
      	   ZMOHO=scrust(kxy)+sediment(kxy)
          !IF(ZMOHO>60.D3) THEN				   !! TIBETAN PLATEAU,  Low viscosity for thicker crust
		!visco(kxy)=visco(kxy)/Reduce_visc	   !! TIBETAN PLATEAU,  Low viscosity for thicker crust
		!Nlow_visc=Nlow_visc+1
	  !ENDIF
      END DO
      IF(Nlow_visc>0) WRITE (6,"(3X,'Viscosity has been reduced by',
     +		F6.2,' in: ',I8,' Points')") Reduce_visc, Nlow_visc
     ...
     +				visco, zname, iwr_filt)			!! Filter Viscosity
      ...
      DO kxy=1,nn
	 !IF(ibody(kxy)/=1) THEN				   !! TIBETAN PLATEAU,  Body 1 = Indenter
	 	visco(kxy)=MIN(visco(kxy),vissup)
	 	visco(kxy)=MAX(visco(kxy),visinf)
	 !ENDIF
      END DO
     ...
     +		' Pa.s,   maxima:',1P,G12.4,' Pa.s')") vismin, vismax     
      ...
      IF (ALLOCATED(x_pol1)) DEALLOCATE(x_pol1,STAT=istat)       
      IF (ALLOCATED(y_pol1)) DEALLOCATE(y_pol1,STAT=istat)       
      IF (ALLOCATED(x_pol2)) DEALLOCATE(x_pol2,STAT=istat)       
      IF (ALLOCATED(y_pol2)) DEALLOCATE(y_pol2,STAT=istat)       
      IF (ALLOCATED(x_pol3)) DEALLOCATE(x_pol3,STAT=istat)       
      IF (ALLOCATED(y_pol3)) DEALLOCATE(y_pol3,STAT=istat)       
      IF (ALLOCATED(x_pol4)) DEALLOCATE(x_pol4,STAT=istat)       
      IF (ALLOCATED(y_pol4)) DEALLOCATE(y_pol4,STAT=istat)       
      IF (ALLOCATED(x_pol5)) DEALLOCATE(x_pol5,STAT=istat)       
      IF (ALLOCATED(y_pol5)) DEALLOCATE(y_pol5,STAT=istat)       
...
!!  outp.seds:  sediments que surten per les vores.
     ...
     +		      GLit_ter, Tmoho, TISOTER, ZASTH, Delta_sed_eros)
      ...
      CHARACTER*2 file_ext2
      CHARACTER*3 file_ext3
      CHARACTER*15 file_st,file_bas,file_xyw, file_tisc, file_lak
...
!  Definition 'tao' arrays, topo_tisc, sedi_tisc:
	 ...
	 DO ix=0,n
	     ...
	     q=roc*g*(scrust(kxy)-scrust0(kxy))+
     +			(rosed*g*sediment(kxy))+
	 ...
	 file_ext3=CHAR(nres_cent)//CHAR(nres_dec)//CHAR(nres_unit) 
         ...
         IF(nres_cent==48) THEN
            IF(nres_dec==48) THEN
              file_tisc='results_tisc'//file_ext1
              file_st='res_tisc'//file_ext1//'.st'
              file_xyw='res_tisc'//file_ext1//'.xyw'
	      file_lak='res_tisc'//file_ext1//'.lak'
	      file_bas='res_tisc'//file_ext1//'.bas'
            ELSE
              file_tisc='results_tisc'//file_ext2
              file_st='res_tisc'//file_ext2//'.st'
              file_xyw='res_tisc'//file_ext2//'.xyw'
	      file_lak='res_tisc'//file_ext2//'.lak'
	      file_bas='res_tisc'//file_ext2//'.bas'
            ENDIF
         ELSE
           file_tisc='results_tisc'//file_ext3
           file_st='res_tisc'//file_ext3//'.st'
           file_xyw='res_tisc'//file_ext3//'.xyw'
	   file_lak='res_tisc'//file_ext3//'.lak'
	   file_bas='res_tisc'//file_ext3//'.bas'
         ENDIF
...
!!	    IF(exist_file) print *, system ('mv res_tisc.xyw '//file_xyw)
          INQUIRE(FILE='res_tisc.lak',EXIST=exist_file)
	    IF(exist_file) call system ('mv res_tisc.lak '//file_lak)
          INQUIRE(FILE='res_tisc.bas',EXIST=exist_file)
	    IF(exist_file) call system ('mv res_tisc.bas '//file_bas)
...
! Move the model to the south, deleting the firts row (iy=0) and adding a new row to the north
     ...
     +		TotalStrain(nn), erosion(nn), Tmoho(nn), 
     +		Qsurface(nn), TEMPE(nn,0:NELZ), 
...
!       END DO
	...
	DO iy=0,m-1
	   ...
	   TotalStrain(kxy)=TotalStrain(kxy_sup)
	...
	END DO
      ...
      DO ix=0,n	!! iy=m
	   ...
	   TotalStrain(kxy)=TotalStrain(kxy_inf)
	   ...
	   END DO
      END DO
!!      DO ix=0,n	!! iy=m		New line of Erosion and Sedimentation fixed to Zero
!!	   iy=m
!!	   kxy=ix+1+iy*(n+1)
!!	   sediment(kxy)=0.D0
!!	   erosion(kxy)=0.D0
